...
Up to this point the reduction has placed a single GPT system entirely inside $\mathbf{Vec}$ \al{I have a couple questions about this: Clearly objects are labelled by vector spaces plus some extra data. There is therefore naturally a forgetful functor into Vec that forges the extra cone data. Two GPT objects however may land on the same Vec object. Probably this collapse of data is no big issue, as whether a functor is surjective or injective, is usually asked at the level of morphisms. That said, is it obvious that the structure of maps is the same? the positivity conditions doesn't change the morphisms enough so that it becomes different from the Vec category?}. The monoidal structure is where the two frameworks genuinely diverge. Recall from the construction of $\square$ that the universal map $\phi_\square:V_A\otimes V_B\to V_{A\square B}$ is injective, yielding the decomposition
\[
V_{A\square B}\;\cong\;(V_A\otimes V_B)\oplus C .
\]
...
\otimes_{\min}\;\subseteq\;V_{A\square B}^+\;\subseteq\;\otimes_{\max},
\]
where $\otimes_{\min}$ is the cone generated by product states $v\square v'$ \al{should we ise the tensor product, $\otimes$ here? I'm just asking since it seemed acted at the level of objects, aka it takes two vector \textit{spaces} and spits out a third one, but was not actually a map that takes or spits out \textit{vectors} themselves.} (the separable cone) and $\otimes_{\max}$ is the cone of all elements positive against every product effect. Summarizing, the square product decomposes into three pieces of data:
\[
...
\]
\al{what is that choice of composite cone?}Only the first piece exists in $\mathbf{Vec}$; the latter two are exactly the additional structure that a GPT carries.
