...
% 2026-09-11 (agent, on Pedro's instruction): B3's closing question replaced by
% the sharper one, in Pedro's own words of 2026-09-11 ("what does a
% probabilistic theory require to be distributed over space?"). Drafted,
% awaiting approval; the rest of B3 is approved 2026-09-01 and untouched.
% "Space" rather than "spacetime" is deliberate (Pedro, 2026-09-11):
% distributed over space, then evolving in time. Old wording, approved
% 2026-09-01, kept here for reversion:
...
% theory?
% Not "a more precise question" (Pedro, 2026-09-11): the new question is not a
% refinement of no-signalling's, it is orthogonal to that principle. The lead-in
% is therefore the one already approved on 2026-09-01, "This reasoning then
% suggests a natural question", with only the question itself changed.
...
theory. This reasoning then suggests a natural question: what does a
probabilistic theory require in order to be distributed over space?}

% New beginning (2026-09-01): NS = kinematical picture (points at spacelike
% separation) vs locality in TFT. Draft, awaiting approval.
% 2026-09-11 (agent, on Pedro's instruction): the question was moved from the
% end of this block to the end of B3, where it follows the compatibility
% sentence directly. This block now ends on the causal relation between a pair
% of points.
...
is the causal relation between a pair of points.}

% 2026-09-11 (agent, on Pedro's instruction): the locality-of-action sentence
% removed from B4; the beat now ends on the question. Kept for salvage:
% Another crucial feature of spacetime physics is the \emph{locality of
% action}~\cite{Einstein1948}: between points that \emph{are} causally
% connected, influences propagate locally through spacetime, by a dynamical
% law.
